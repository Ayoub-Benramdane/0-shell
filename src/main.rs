...
fn main() -> rustyline::Result<()> {
    ...
    loop {
        ...
        let mut path = if let Ok(current) = current_place && !cmd_map.contains_key(&Commands::Pwd) {
        ...
        let mut input_splited = tokenize_input(&line, &user);
        
        for command in input_splited.iter_mut() {
            let mut n_cmd = command[0].clone();
            n_cmd = n_cmd.replace("\'", "");
            n_cmd = n_cmd.replace("\"", "");
            match Commands::from_str(&n_cmd) {
                Some(cmd) => {
                    execute(cmd, &mut command[1..].to_owned(), &mut cmd_map);
                }
                None => {
