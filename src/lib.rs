...
pub mod zero {
    ...
    pub fn execute(
        ...
        match cmd {
            ...
            Commands::Mv => {
                exec_mv(cmd, args, mp);
                mp.remove(&Commands::Mv);
            }
            Commands::Pwd => { exec_pwd(cmd, args, mp) }
            Commands::Mkdir => {
                exec_mkdir(cmd, args, mp);
                mp.remove(&Commands::Mkdir);
            }
            Commands::Cp => {
                exec_cp(cmd, args, mp);
                mp.remove(&Commands::Cp);
            }
            Commands::Exit => {
                exec_exit(args);
                mp.remove(&Commands::Exit);
            }
            Commands::Cat => {
                let _ = exec_cat(&mut catt, cmd, args, mp);
                mp.remove(&Commands::Cat);
            }
            Commands::Clear => {
                exec_clear(cmd, args, mp);
                mp.remove(&Commands::Clear);
            }
            Commands::History => {
                exec_history(cmd, args, mp);
                mp.remove(&Commands::History);
            }
            Commands::Ls => {
                exec_ls(cmd, args, mp);
                mp.remove(&Commands::Ls);
            }
            Commands::Touch => {
                exec_touch(cmd, args, mp);
                mp.remove(&Commands::Touch);
            }
