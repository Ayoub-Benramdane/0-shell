use std::fs;
use crate::{commands::ls::create_path, shell_core::*};
    ...
    for src in srcs {
        ...
        match fs::rename(&src_path, &final_dest) {
            ...
            Err(_) => {
                ...
                execute_command(BuiltinCommand::Cp, &mut copy_args, &mut flag_map.clone());
                ...
                if final_dest.exists() {
                    ...
                    execute_command(BuiltinCommand::Rm, &mut rm_args, &mut flag_map.clone());
                } else {
